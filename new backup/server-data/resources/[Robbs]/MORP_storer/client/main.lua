                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                ESX.TriggerServerCallback('ev-storerobbery:server:getOfficers', function(amount)
                                    CurrentCops = amount
                                    if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                        ...
                                        if not copsCalled then
                                            ...
                                            TriggerServerEvent("ev-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                            copsCalled = true
                                        end
                                    else
            ...
            ESX.TriggerServerCallback('ev-storerobbery:server:getOfficers', function(amount)
                ...
                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                    ...
                    if not copsCalled then
                        ...
                        NotifyPolice()
                        TriggerServerEvent("ev-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                        ...
                        Skillbar.Start({
                            ...
                            Skillbar.Start({
                                ...
                                if currentRegister ~= 0 then
                                    lockpick(false)
                                    TriggerServerEvent('ev-storerobbery:server:setRegisterStatus', currentRegister)
                                    ...
                                    Progressbar("search_register", "Empty the cash safe.", lockpickTime, false, true, {
                                        ...
                                        TriggerServerEvent('ev-storerobbery:server:takeMoney', currentRegister, true)            
                                        currentRegister = 0
                                    end, function() -- Cancel
                                        inFuckingSafeRob = false
                                        openingDoor = false
...
function setupRegister()
    ESX.TriggerServerCallback('ev-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    ESX.TriggerServerCallback('ev-storerobbery:server:getSafeStatus', function(Safes)
...
function LockpickDoorAnim(time)
    ...
    Citizen.CreateThread(function()
        while openingDoor do
            ...
            Citizen.Wait(2000)
            time = time - 2
            TriggerServerEvent('ev-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("ev-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("ev-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
function Failed()
    ...
    if math.random(1, 100) < 20 then
        TriggerServerEvent("ev-storerobbery:server:RemoveItem", "lockpick", 1)
    end
...
RegisterNUICallback('TryCombination', function(data, cb)
    ESX.TriggerServerCallback('ev-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('ev-storerobbery:client:setRegisterStatus')
AddEventHandler('ev-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('ev-storerobbery:client:setSafeStatus')
AddEventHandler('ev-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('ev-storerobbery:client:robberyCall')
AddEventHandler('ev-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    local cameraId = 4
    if type == "safe" then
...
function NotifyPolice()
	...
	TriggerServerEvent('policealerts:server:AddPoliceAlert', {
	  timeOut = 20000,
	  alertTitle = "Shop Robbery In Progress",
	  coords = {
		  x = coords.x,
		  y = coords.y,
		  z = coords.z,
