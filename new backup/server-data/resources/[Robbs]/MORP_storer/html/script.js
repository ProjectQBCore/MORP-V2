...
Padlock.Close = function() {
    ...
    $.post('http://MORP_storer/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('http://MORP_storer/PadLockClose');
    if (data.error != null) {
        $.post('http://MORP_storer/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("http://MORP_storer/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle &&
                ...
                // on unlock
                $.post('http://MORP_storer/callcops');
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post('http://MORP_storer/PadLockSuccess');
                    Padlock.Close();
                }
            }
            break;
