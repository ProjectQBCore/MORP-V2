...
AddEventHandler('esx_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
	TriggerEvent('notification', _U('robbery_cancelled'), 1)
	robbingName = ""
	incircle = false
end)
...
AddEventHandler('esx_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	TriggerEvent('notification', _U('robbery_complete'), 1)
	store = ""
	incircle = false
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(Stores)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not holdingup then
					--DrawMarker(27, v.position.x, v.position.y, v.position.z-0.9, 0, 0, 0, 0, 0, 0, 2.001, 2.0001, 0.5001, 0, 0, 250, 200, 0, 0, 0, 0)
					DrawText3Ds(v.position.x, v.position.y, v.position.z , 'Shoot to start the robbery')

					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							--DrawText3Ds(v.position.x, v.position.y, v.position.z , '[E] Check in (250$)')
							    ...
							    if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
							        ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
								        if CopsConnected >= Config.RequiredCopsRob then
									        ...
									        TriggerEvent('notification', _U('min_two_police') .. Config.RequiredCopsRob .. _U('min_two_police2'))
								        end
							        end)		
						        else
							        TriggerEvent('notification', _U('need_bag'))
								end
							else
								ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsRob then
		...
		--TriggerEvent('notification', 'Broking the window')
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					DrawText3Ds(v.x, v.y, v.z, '~w~[~g~E~w~] ' .. _U('press_to_collect'), 0.6)
					if IsControlJustPressed(0, 38) then
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerEvent('notification', _U('lester'), 1)
						    holdingup = false
						    StopSound(soundid)
						end
					end
