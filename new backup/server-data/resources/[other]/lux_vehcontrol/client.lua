...
---------------------------------------------------
CONTROLS	
	Right indicator:	=	(Next Custom Radio Track) changed 175 
	Left indicator:		-	(Previous Custom Radio Track) changed 189
	Hazard lights:	Backspace	(Phone Cancel)
	Toggle emergency lights:	Y	(Text Chat Team)
	Airhorn:	E	(Horn) 
	Toggle siren:	,	(Previous Radio Station) changed (siren ctlr) 36
	Manual siren / Change siren tone:	N	(Next Radio Station)
	Auxiliary siren:	Down Arrow	(Phone Up) disabled
---------------------------------------------------
]]
			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 36, true) -- INPUT_DUCK (siren ctlr) 
						...
						if not IsPauseMenuActive() then
							...
							-- POWERCALL
							elseif IsDisabledControlJustReleased(0, 156) then --disabled old key 172 (arrow up)
								if state_pwrcall[veh] == true then
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TogPowercallStateForVeh(veh, false)
									count_bcast_timer = delay_bcast_timer
