...
AddEventHandler('100', function(id,message,name,coords)
	if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance' or id == GetPlayerServerId(PlayerId()) then
		...
		if id ~= GetPlayerServerId(PlayerId()) then
			...
			AddTextComponentSubstringPlayerName('EMERGENCY: ' .. name .. ' Police Call')
...
AddEventHandler('101', function(id,message,name,coords)
	if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance' or id == GetPlayerServerId(PlayerId()) then
		PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
		...
		if id ~= GetPlayerServerId(PlayerId()) then
			--local blip = AddBlipForEntity(GetPlayerPed(GetPlayerFromServerId(id))) old 
			local blip = AddBlipForCoord(coords)
			...
			BeginTextCommandSetBlipName('STRING')
			AddTextComponentSubstringPlayerName('EMERGENCY: ' .. name .. ' EMS Call')
			EndTextCommandSetBlipName(blip)
...
--[[
RegisterNetEvent('deadblip:receivecrds')
AddEventHandler('deadblip:receivecrds', function(crds)
	print(crds)
end)
]]