...
ESX.RegisterServerCallback('inventory:doesInvTypeExists', function(source, cb, type)
    cb(InvType[type] ~= nil)
end)
...
RegisterServerEvent('inventory:deleteNilInventory')
AddEventHandler('inventory:deleteNilInventory', function()
    local owner = ESX.GetPlayerFromId(source).identifier
    MySQL.Async.fetchAll('DELETE FROM disc_inventory WHERE data = @data AND owner = @owner', { ['@data'] = "null", ['@owner'] = owner })
end)