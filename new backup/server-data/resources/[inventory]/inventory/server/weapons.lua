    ...
    MySQL.Async.fetchAll('SELECT * FROM items WHERE LCASE(name) LIKE \'%weapon_%\'', {}, function(results)
        for k, v in pairs(results) do
            ESX.RegisterUsableItem(v.name, function(source)
                TriggerClientEvent('inventory:useWeapon', source, v.name)
            end)
        end
    end)
end)

RegisterServerEvent('inventory:updateAmmoCount')
AddEventHandler('inventory:updateAmmoCount', function(hash, count)
...
ESX.RegisterServerCallback('inventory:getAmmoCount', function(source, cb, hash)
    local player = ESX.GetPlayerFromId(source)
