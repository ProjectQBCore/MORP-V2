...
Citizen.CreateThread( function()
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			if (GetPedInVehicleSeat(vehicle, -1) == ped) and IsVehicleOnAllWheels(vehicle) and IsControlJustReleased(0, 43) and IsVehicleClassWhitelisted(GetVehicleClass(vehicle)) then
				ToggleDrift()
			end
		end
	end
