...
RegisterNetEvent('ev-houseRobbery:attempt')
AddEventHandler('ev-houseRobbery:attempt', function(lockpicks)
 if isRobbing and DoesEntityExist(safe) and not safeOpen then
  ...
  if GetDistanceBetweenCoords(playerCoords, safepos.x, safepos.y, safepos.z, true) <= 3.0 then
   TriggerEvent('ev-lockpick:client:openLockpick', function(success)
      if success then
         safeOpen = true
         TriggerServerEvent('ev-houseRobbery:giveMoney')
      else
         NotifyPolice()
      end
   end)
   ...
   if math.random(1, 20) == 1 then 
    TriggerServerEvent('ev-houseRobbery:removeLockpick')
   end
  end
 end
   ...
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
      ...
      TriggerEvent('ev-lockpick:client:openLockpick', function(success)
         ...
         if success then
            ...
            TriggerEvent('ev-houseRobbery:createHouse', id)
         else
            NotifyPolice()
...
RegisterNetEvent('ev-houseRobbery:createHouse')
AddEventHandler('ev-houseRobbery:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   ...
   if IsControlJustPressed(0, 38) then
    TriggerEvent('ev-houseRobbery:deleteHouse', id)
   end
  end
 end
end)
...
RegisterNetEvent('ev-houseRobbery:deleteHouse')
AddEventHandler('ev-houseRobbery:deleteHouse', function(id)
 local house = robbableHouses[id]
  ...
  if isRobbing then
   ...
   local Skillbar = exports['skillbar']:GetSkillbarObject()
      ...
      Skillbar.Start({
         ...
         TriggerServerEvent('ev-houseRobbery:searchItem')
     end, function()
      ClearPedTasks(PlayerPedId())
     end)
...
function NotifyPolice()
 ...
 local PawnLocation = vector3(412.05673217773, 315.18167114258, 103.13388061523)
 local PawnItems = {
   {name = 'samsung_s8', label = 'Samsung S8', price = 45}, 
   {name = 'sunglasses', label = 'Sunglasses', price = 45}, 
   {name = 'apple_iphone', label = 'Apple iPhone', price = 100}, 
   {name = 'nokia_phone', label = 'Nokia Phone', price = 30}, 
   {name = 'ring', label = 'Ring', price = 55}, 
   {name = 'camera', label = 'Camera', price = 45}, 
   {name = 'gameboy', label = 'Gameboy', price = 35}, 
   {name = 'casio_watch', label = 'Casio Watch', price = 35}, 
   {name = 'white_pearl', label = 'White Pearl', price = 50}, 
   {name = 'psp', label = 'PSP', price = 45}, 
   {name = '2ct_gold_chain', label = '2CT Gold Chain', price = 100},
   {name = '5ct_gold_chain', label = '5CT Gold Chain', price = 115},
   {name = '8ct_gold_chain', label = '8CT Gold Chain', price = 120},
   {name = '10ct_gold_chain', label = '10CT Gold Chain', price = 180},
   {name = 'pearl_necklace', label = 'Pearl Necklace', price = 190},
   {name = 'electronics', label = 'Electronics', price = 40},
   {name = 'breadboard', label = 'Breadboard', price = 50},
}


 CreateThread(function()

   local blip = AddBlipForCoord(PawnLocation)
   ...
   while true do
      Wait(1)
      local d = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), PawnLocation[1], PawnLocation[2], PawnLocation[3])
      if d < 1.5 then
         DrawText3Ds(PawnLocation[1], PawnLocation[2], PawnLocation[3], 'Press E to open Pawn Shop')
         if IsControlJustPressed(0,38) then
            SellMenu()
         end
      elseif d < 4.5 then
         DrawText3Ds(PawnLocation[1], PawnLocation[2], PawnLocation[3], 'Pawn Shop')
      end
 ...
 function SellMenu()
	...
	for k,v in pairs(PawnItems) do
		table.insert(elements,{label = v.label .. " | "..('<span style="color:green;">%s</span>'):format("$"..v.price..""), name = v.name, price = v.price})
	end
	...
	function(data, menu)
		if data.current.itemName == data.current.itemName then
			SellDialog(data.current.name,data.current.price)
		end	
...
function SellDialog(itemName, SellPrice)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_dialog', {
		title = "Amount"
	}, function(data, menu)
		menu.close()
		amountToSell = tonumber(data.value)
		totalSellPrice = (SellPrice * amountToSell)
		TriggerServerEvent("robbery:SellItem",amountToSell,totalSellPrice,itemName)
	end,
	function(data, menu)
		menu.close()	
	end)
