        ...
        while (i > 0) {
            ...
            setTimeout(function() {
                ...
                if (game_state.win == game_state.paid) {
                    $.post('http://MORP_slot/payforplayer', JSON.stringify(game_state.win));
...
function post(name, data) {
    $.post('http://MORP_slot/' + name, JSON.stringify(data));
}
...
var spin_handler = function() {
    if (freespins <= 0) {
        if (game_state.spin_click_shield) {
            ...
            $.post('http://MORP_slot/playerpays', JSON.stringify(playerbet));
