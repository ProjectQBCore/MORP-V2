...
AddEventHandler('esx:playerLoaded', function(playerData)
    ...
    TriggerServerEvent("mvd:server:UpdateBlips")
    TriggerServerEvent("mvd:server:UpdateCurrentCops")
    TriggerServerEvent("mvd:server:CheckBills")
...
RegisterNetEvent('mvd:client:toggleDatabank')
AddEventHandler('mvd:client:toggleDatabank', function()
...
local DutyBlips = {}
RegisterNetEvent('mvd:client:UpdateBlips')
AddEventHandler('mvd:client:UpdateBlips', function(players)
    ...
    if (ESX.PlayerData.job ~= nil and players[GetPlayerServerId(PlayerId())]) and (ESX.PlayerData.job.name == 'mvd' or ESX.PlayerData.job.name == 'ambulance') then
        if players ~= nil then
            for k, data in pairs(players) do
                local id = GetPlayerFromServerId(data.source)
                if NetworkIsPlayerActive(id) then
                    if data.job.name == 'mvd' then
...
RegisterNetEvent('mvd:PlaySound')
AddEventHandler('mvd:PlaySound', function()
    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
end)

RegisterNetEvent('mvd:client:mvdEmergencyAlert')
AddEventHandler('mvd:client:mvdEmergencyAlert', function(callsign, streetLabel, coords)
    if (ESX.PlayerData.job.name == 'mvd' or ESX.PlayerData.job.name == 'ambulance') then
        local transG = 250
...
RegisterNetEvent('mvd:server:SendEmergencyMessageCheck')
AddEventHandler('mvd:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    local PlayerData = ESX.GetPlayerData()
    if ((PlayerData.job.name == "mvd" or PlayerData.job.name == "ambulance")) then
        ...
        TriggerEvent("mvd:client:EmergencySound")
        local transG = 250
        local blip = AddBlipForCoord(coords.x, coords.y, coords.z)
...
RegisterNetEvent('mvd:client:SendToJail')
AddEventHandler('mvd:client:SendToJail', function(time)
    TriggerServerEvent("mvd:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(GetPlayerPed(-1))
...
AddEventHandler('impound', function()
    ...
    if not IsPedInAnyVehicle(ped, false) and DoesEntityExist(vehicle) and ( Vdist2(GetEntityCoords(vehicle), coords) < 5 ) then
        ...
        if ( Vdist2(GetEntityCoords(vehicle), coords) < 5 ) then
            TriggerServerEvent("mvd:server:impound", GetVehicleNumberPlateText(vehicle))
...
AddEventHandler('esx_mvdjob:OutVehicle', function()
...
RegisterNetEvent("mvd:out:player")
AddEventHandler("mvd:out:player",function()
                local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_mvdjob:OutVehicle', GetPlayerServerId(closestPlayer))
