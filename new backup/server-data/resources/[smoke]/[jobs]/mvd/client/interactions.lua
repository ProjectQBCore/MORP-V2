...
RegisterNetEvent('mvd:client:SetOutVehicle')
AddEventHandler('mvd:client:SetOutVehicle', function()
...
RegisterNetEvent('mvd:client:PutInVehicle')
AddEventHandler('mvd:client:PutInVehicle', function()
...
RegisterNetEvent('mvd:client:checkBank')
AddEventHandler('mvd:client:checkBank', function()
	t, distance, closestPed = ESX.Game.GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("mvd:targetCheckBank", GetPlayerServerId(t))
	else
		TriggerEvent("notification", "No player near you!",2)
	end
end)

RegisterNetEvent('mvd:client:SearchPlayer')
AddEventHandler('mvd:client:SearchPlayer', function()
...
RegisterNetEvent('mvd:client:SeizeCash')
AddEventHandler('mvd:client:SeizeCash', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("mvd:server:SeizeCash", playerId)
    else
        TriggerEvent('notification', "No player near you!", 2)
    end
end)

RegisterNetEvent('mvd:client:SeizeDriverLicense')
AddEventHandler('mvd:client:SeizeDriverLicense', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("mvd:server:SeizeDriverLicense", playerId)
    else
        TriggerEvent('notification', "No player near you!", 2)
    end
end)
...
RegisterNetEvent('mvd:client:RobPlayer')
AddEventHandler('mvd:client:RobPlayer', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
...
RegisterNetEvent('mvd:client:JailCommand')
AddEventHandler('mvd:client:JailCommand', function(playerId, time)
    TriggerServerEvent("mvd:server:JailPlayer", playerId, tonumber(time))
end)

RegisterNetEvent('mvd:client:BillCommand')
AddEventHandler('mvd:client:BillCommand', function(playerId, price)
    TriggerServerEvent("mvd:server:BillPlayer", playerId, tonumber(price))
end)

RegisterNetEvent('mvd:client:JailPlayer')
AddEventHandler('mvd:client:JailPlayer', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
...
RegisterNetEvent('mvd:client:BillPlayer')
AddEventHandler('mvd:client:BillPlayer', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        DisplayOnscreenKeyboard(1, "", "", "", "", "", "", 20)
...
RegisterNetEvent('mvd:client:PutPlayerInVehicle')
AddEventHandler('mvd:client:PutPlayerInVehicle', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if (not isHandcuffed and not isEscorted) then
            TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "set " .. GetPlayerName(player) .. "(" .. playerId .. ") in vehicle")
            TriggerServerEvent("mvd:server:PutPlayerInVehicle", playerId)
        end
    else
        TriggerEvent('notification', "No player near you!", 2)
    end
...
RegisterNetEvent('mvd:client:SetPlayerOutVehicle')
AddEventHandler('mvd:client:SetPlayerOutVehicle', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "set " .. GetPlayerName(player) .. "(" .. playerId .. ") out of vehicle")
            TriggerServerEvent("mvd:server:SetPlayerOutVehicle", playerId)
        end
    else
        TriggerEvent('notification', "No player near you!", 2)
    end
...
RegisterNetEvent('mvd:client:EscortPlayer')
AddEventHandler('mvd:client:EscortPlayer', function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Escort " .. GetPlayerName(player) .. "(" .. playerId .. ")")
            TriggerServerEvent("mvd:server:EscortPlayer", playerId)
        end
    else
        TriggerEvent('notification', "No player near you!", 2)
    end
...
AddEventHandler('mvd:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Is robbing " .. GetPlayerName(player) .. "(" .. playerId .. ")")
                TriggerServerEvent("mvd:server:KidnapPlayer", playerId)
            end
        end
    else
        TriggerEvent('notification', "No player near you!", 2)
...
RegisterNetEvent('mvd:client:fineMenu')
AddEventHandler('mvd:client:fineMenu', function()
...
RegisterNetEvent('mvd:client:CuffPlayerSoft')
AddEventHandler('mvd:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        local player, distance = ESX.Game.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim ( GetPlayerPed(player), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "random@arrests@busted", "idle_a", 3) or ESX.PlayerData.job.name == 'mvd') then
                TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Cuff (Soft) " .. GetPlayerName(player) .. "(" .. playerId .. ")")
                TriggerServerEvent("mvd:server:CuffPlayer", playerId, true)
                HandCuffAnimation()
            else
...
RegisterNetEvent('mvd:client:CuffPlayer')
AddEventHandler('mvd:client:CuffPlayer', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        local player, distance = ESX.Game.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim ( GetPlayerPed(player), "random@mugging3", "handsup_standing_base", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "random@arrests@busted", "idle_a", 3) or ESX.PlayerData.job.name == 'mvd') then
                TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Cuff (Hard) " .. GetPlayerName(player) .. "(" .. playerId .. ")")
                TriggerServerEvent("mvd:server:CuffPlayer", playerId, false)
                HandCuffAnimation()
            else
                TriggerEvent("notification", "You can only cuff players that have their hands up.",2)
            end
...
RegisterNetEvent('mvd:client:UnCuffPlayer')
AddEventHandler('mvd:client:UnCuffPlayer', function()
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        local player, distance = ESX.Game.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if (not isHandcuffed and not isEscorted) and (IsEntityPlayingAnim(GetPlayerPed(player), "mp_arresting", "idle", 3) or IsEntityPlayingAnim(GetPlayerPed(player), "mp_arrest_paired", "crook_p2_back_right", 3) or ESX.PlayerData.job.name == 'mvd') then
                TriggerServerEvent('mvd:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") " .. "Uncuffed " .. GetPlayerName(player) .. '(' .. playerId .. ')') 
                TriggerServerEvent('mvd:server:UnCuffPlayer', playerId)
                UnHandCuffAnimation()
            else
...
RegisterNetEvent('mvd:client:GetEscorted')
AddEventHandler('mvd:client:GetEscorted', function(playerId)
...
RegisterNetEvent('mvd:client:DeEscort')
AddEventHandler('mvd:client:DeEscort', function()
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(GetPlayerPed(-1), true, false)
end)

RegisterNetEvent('mvd:client:GetKidnappedTarget')
AddEventHandler('mvd:client:GetKidnappedTarget', function(playerId)
...
RegisterNetEvent('mvd:client:GetKidnappedDragger')
AddEventHandler('mvd:client:GetKidnappedDragger', function(playerId)
...
RegisterNetEvent('mvd:client:GetCuffed')
AddEventHandler('mvd:client:GetCuffed', function(playerId, isSoftcuff)
    isSoftcuff = isSoftcuff or true
    isHandcuffed = true
    TriggerServerEvent("mvd:server:SetHandcuffStatus", true)
    ClearPedTasksImmediately(GetPlayerPed(-1))
...
RegisterNetEvent('mvd:client:GetUnCuffed')
AddEventHandler('mvd:client:GetUnCuffed', function(playerId)
    isHandcuffed = false
    TriggerServerEvent("mvd:server:SetHandcuffStatus", false)
    ClearPedTasksImmediately(GetPlayerPed(-1))
...
function IsTargetDead(playerId)
    local retval = false
    ESX.TriggerServerCallback('mvd:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
...
RegisterCommand('remask', function()
    TriggerEvent('mvd:remmask')
end)

RegisterNetEvent('mvd:remmaskAccepted')
AddEventHandler('mvd:remmaskAccepted', function()
	TriggerEvent("mvd:facewearAdjust", 1, true)
	TriggerEvent("mvd:facewearAdjust", 3, true)
	TriggerEvent("mvd:facewearAdjust", 4, true)
	TriggerEvent("mvd:facewearAdjust", 2, true)
end)

RegisterNetEvent('mvd:remmask')
AddEventHandler('mvd:remmask', function()
	...
	if (distance ~= -1 and distance < 2) then
		if isOppositeDir(GetEntityHeading(t),GetEntityHeading(PlayerPedId())) and not IsPedInVehicle(t,GetVehiclePedIsIn(t, false),false) then
			TriggerServerEvent("mvd:remmaskGranted", GetPlayerServerId(t))
...
RegisterNetEvent("mvd:facewearAdjust")
AddEventHandler("mvd:facewearAdjust",function(faceType,remove)
	local AnimSet = "none"
