		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					vehicleProps.vehiclemodel = model
					vehicleProps.vehiclemodelname = GetLabelText(model)
					...
					TriggerServerEvent('garage:addKeys', GetPlayerServerId(closestPlayer), GetVehicleNumberPlateText(currentDisplayVehicle))
					TriggerServerEvent('garage:addKeys', GetVehicleNumberPlateText(currentDisplayVehicle))
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								local newPlate = 'MORP' .. string.upper(ESX.GetRandomString(4))
