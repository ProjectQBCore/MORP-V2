			...
			if nearestDist < self.DrawTextDist then
				...
				if (IsControlJustPressed(0, JUtils.Keys["E"], IsDisabledControlJustPressed(0, JUtils.Keys["E"]))) then 
					...
					while istrue do
						...
						if (nearestDistB < self.DrawTextDist and nearestVehB == nearestVeh and plyJob == self.CarDealerJobLabel) then 
                ...
                if plyJob == self.CarDealerJobLabel then
                  ...
                  self.CurBuying = true
									if valid then 
										...
										TriggerEvent('notification', 'You have purchased this vehicle!')
										local spawnPos
										...
										ESX.Game.SpawnVehicle(nearestModel,spawnPos.xyz, spawnPos.w, function(cbVeh)
											...
											TriggerServerEvent('cardealer:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") just bought " .. nearestModel .. ".")
                      self.CurBuying = false
										end)
									else
										TriggerEvent('notification', "You don't have enough money for this purchase.")
				...
				elseif (IsControlJustPressed(0, JUtils.Keys["F"], IsDisabledControlJustPressed(0, JUtils.Keys["F"]))) and plyJob == self.CarDealerJobLabel then 
					...
					if type(extraStr) == "number" then
						if plyJob == self.CarDealerJobLabel then
							...
							self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 1.0, "[ "..vehName.." ]")
							self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 0.9, "[ ~r~"..extraStr.."~s~% Commission ]")
						end
					else						
						if plyJob == self.CarDealerJobLabel then
							self:DrawText3D(nearestPos.x,nearestPos.y,nearestPos.z + 1.0, "[ "..vehName.." ] : [ $" .. vehPrice .. " ] : " .. extraStr)
...
function JVS:TestDriveVehicle(model, listtype)
	...
	TriggerServerEvent('cardealer:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") just taked " .. model .. " to test drive.")
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if self.TestingCar and self.TestingList then
			...
			TriggerServerEvent('cardealer:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") just return test drive " .. GetDisplayNameFromVehicleModel(GetEntityModel(self.TestingCar)) .. " to catalog.")
            ...
            if DoesEntityExist(self.TestingCar) then SetVehicleUndriveable(self.TestingCar, true); end
						TriggerEvent('notification', "You have returned the test vehicle.")
	...
	ESX.TriggerServerCallback('JAM_VehicleShop:PurchaseVehicle', function(valid)
		if valid then 
			...
			ESX.Game.SpawnVehicle(veh.model, self.PurchasedUtilPos.xyz, self.PurchasedUtilPos.w, function(cbVeh)
				...
				TriggerServerEvent('JAM_VehicleShop:CompletePurchase', vehProps)
				TriggerEvent("keys:addNew",cbVeh,GetVehicleNumberPlateText(cbVeh))
				TriggerServerEvent('cardealer:logs', GetPlayerName(PlayerId()) .. "(" .. GetPlayerServerId(PlayerId()) .. ") just bought " .. veh.model .. ".")
			end)
		else
			TriggerEvent('notification', "You don't have enough money for this purchase.", 2)
		end
		istrue = false
...
function JVS:OpenCategoryMenu(category)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Category_Menu", { title = category.label, align = 'left', elements = elements }, 
		function(data,menu)
			menu.close()
...
function JVS:OpenDealerMenu()
	local plyData = ESX.GetPlayerData()
	if plyData.job.grade >= 2 then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Dealer_Menu", { title = "Vehicle Shop", align = 'left', elements = { [1] = {label = "Rearrange Displays"}, [2] = {label = "Dealership Account"} } }, 
			function(data,menu)
				menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), "Dealer_Menu", { title = "Vehicle Shop", align = 'left', elements = { [1] = {label = "Rearrange Displays"} } }, 
			function(data,menu)
				menu.close()
						...
						function(data2, menu2)
							...
							if quantity == nil then
								...
								ESX.TriggerServerCallback('JAM_VehicleShop:DepositDealerMoney', function(valid)
									if valid then TriggerEvent('notification', "You deposited $~g~"..quantity.."~s~ into the inventory.")
									else TriggerEvent('notification', "Invalid quantity.", 2)
									end
								end, quantity)
                menu.close()
                self.MenuOpen = false
						...
						function(data2, menu2)
							...
							if quantity == nil then
								...
								ESX.TriggerServerCallback('JAM_VehicleShop:WithdrawDealerMoney', function(valid)
									if valid then TriggerEvent('notification', "You withdrew $~g~"..quantity.."~s~ from the inventory.", 2)
									else TriggerEvent('notification', "Invalid quantity.", 2)
									end
								end, quantity)
                menu.close()
