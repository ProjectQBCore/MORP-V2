			...
			if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.0)  then
				if not inMission then
					...
					if IsControlJustReleased(0, 38) then
						ESX.TriggerServerCallback('esx_deliverjob:getPlayers', function(plys)
							if plys > 0 then
								TakeDeliver()
							else
								TriggerEvent('notification', 'No deliveries at this moment', 2)
							end
						end)
					end
				else
...
function TakeDeliver()
	...
	--SetEntityCoords(PlayerPedId(),MissionCoords.x,MissionCoords.y,MissionCoords.z)
	...
	Mblip = CreateFuckingBlip(MissionCoords.x, MissionCoords.y, MissionCoords.z)
...
function CreateFuckingBlip(x,y,z)
...
Citizen.CreateThread(function()
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 30.0 then
			DrawText3D(coords[1], coords[2], coords[3], "[E] to deliver the food")
			if IsControlJustReleased(0, 38) and d < 3.0 then
...
function SpawnVehicle()
	if not DoesEntityExist(jobVeh) then
		...
		while not DoesEntityExist(jobVeh) do Wait(1) end
		StartVehicleMarker()
	end
end

function StartVehicleMarker()
	while DoesEntityExist(jobVeh) do Wait(1)
		if (GetVehiclePedIsIn(PlayerPedId()) ~= jobVeh) and inMission then
			local c = GetEntityCoords(jobVeh)
			DrawMarker(2,c.x,c.y,c.z+1, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 100, 15, 15, 130, 0, 0, 0, 0)
		end
	end
end
