		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 100) * 10
					...
					elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
						price = math.floor(vehiclePrice * v.price[data.current.modNum + 1] / 1000) * 10
						TriggerServerEvent("esx_lscustom:buyMod", price)
						if Config.EnableJobLogs then
							TriggerServerEvent('esx_joblogs:AddInLog', "mechanic", "buy_mod" , GetPlayerName(PlayerId()), price, vehicleData.plate)
						end
					elseif v.modType == 17 then
						price = math.floor(vehiclePrice * v.price[1] / 1000) * 10
						TriggerServerEvent("esx_lscustom:buyMod", price)
						if Config.EnableJobLogs then
							TriggerServerEvent('esx_joblogs:AddInLog', "mechanic", "buy_mod", GetPlayerName(PlayerId()), price, vehicleData.plate)
						end
					else
						price = math.floor(vehiclePrice * v.price / 1000) * 10
						TriggerServerEvent("esx_lscustom:buyMod", price)
						if Config.EnableJobLogs then
							TriggerServerEvent('esx_joblogs:AddInLog', "mechanic", "buy_mod", GetPlayerName(PlayerId()), price, vehicleData.plate)
						end
