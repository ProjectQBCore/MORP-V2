...
function SendDistressSignal()
	...
	TriggerServerEvent('esx_addons_gcphone:startCall', 'ambulance', _U('distress_message'), PlayerCoords, { 	--
		PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z }, 							-- disabled
	})																											--
end]]
...
function StartDeathTimer()
	...
	Citizen.CreateThread(function()
		...
		while earlySpawnTimer > 0 and isDead do
			Citizen.Wait(0)
			drawTxt(0.89, 1.44, 1.0,1.0,0.6, "Respawn: ~r~" .. math.ceil(earlySpawnTimer) .. "~w~ seconds remaining (type /101 to call EMS)", 255, 255, 255, 255)
		end
		...
		while isDead do
			...
			drawTxt(0.89, 1.44, 1.0,1.0,0.6, "~w~HOLD ~r~E ~w~(" .. math.ceil(EHeld/100) .. ") ~w~TO ~r~RESPAWN ~w~OR WAIT FOR ~r~EMS (type /101 to call EMS)", 255, 255, 255, 255)
		end
