...
function MenuGarage(action)
    ...
    DeleteActualVeh()
    if action == "menu" then
        Menu.addButton("Vehicles","ListeVehicule",nil)
   ...
   Menu.addButton("return","MenuGarage",nil)
    for c,v in pairs(fueravehicles) do
        ...
        if v.state == 0 or v.state == false then
            Menu.addButton("RECOVER | "..GetDisplayNameFromVehicleModel(vehicle.model), "spawnNow", vehicle, "IN RECOVERY", " Engine : " .. round(vehicle.engineHealth) /10 .. "%", " Fuel : " .. round(vehicle.fuelLevel) .. "%","SpawnLocalVehicle")
        end
    end 
end

function spawnNow(data)
    ESX.TriggerServerCallback('garage:checkMoney', function(hasEnoughMoney)
        ...
        elseif hasEnoughMoney == "deudas" then
            recuperar()
            TriggerEvent('notification', 'You have a debt with the government greater than $2000, you will not be able to recover your vehicle until you pay your fines!', 2)
        else
            recuperar()
