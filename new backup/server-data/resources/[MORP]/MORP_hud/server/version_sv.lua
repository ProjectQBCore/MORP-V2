...
AddEventHandler('onResourceStart', function(resourceName)
	if GetCurrentResourceName() == resourceName then
		function checkVersion(error, latestVersion, headers)
			...
			if --[[currentVersion ~= latestVersion and tonumber(currentVersion) < tonumber(latestVersion)]] false then
			...
			elseif --[[tonumber(currentVersion) > tonumber(latestVersion)]] false then
				print(name .. " has skipped the latest version ^2" .. latestVersion .. ". Either Github is offline or the version file has been changed")
			else
				print(name .. " is Discontinued. (Temp Fix By Smoke Slickz) ^3Settings saved values to players...")
				Wait(3000)
				print("^2Finished")
			end
		end
