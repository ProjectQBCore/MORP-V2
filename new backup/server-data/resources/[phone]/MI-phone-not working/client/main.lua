...
RegisterNUICallback('GetWhatsappChat', function(data, cb)
    if PhoneData.Chats[data.phone_number] ~= nil then
        cb(PhoneData.Chats[data.phone_number])
    else
        cb(false)
    end
end)
...
RegisterNetEvent('MI-phone:updateFaturalar')
AddEventHandler('MI-phone:updateFaturalar', function()
  ESX.TriggerServerCallback('MI-phone:getBills', function(bills)
      setFaturalar(bills)
  end)
end)

RegisterNUICallback('faturapayBill', function (data, cb)
  TriggerServerEvent('MI-phone:faturapayBill', data)
  cb()
end)

RegisterNUICallback('getFaturalar', function (data, cb)
  TriggerEvent('MI-phone:updateFaturalar')
  cb()
end)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
	...
	TriggerServerEvent('sendDisTwT', TweetMessage)
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCallState', function(CanCall, IsOnline)
        ...
        cb(status)
        if CanCall and not status.InCall and (data.ContactData.number ~= PhoneData.PlayerData.charinfo.phone_number) then
...
CallContact = function(CallData, AnonymousCall)
    ...
    PhoneData.CallData.CallId = GenerateCallId(PhoneData.PlayerData.charinfo.phone_number, CallData.number)
...
CancelCall = function()
	...
	local getPlayerRadioChannel = exports.tokovoip_morp:getPlayerData(playerName, "radio:channel")
    if PhoneData.CallData.CallType == "ongoing" then
        --exports["mumble-voip"]:SetCallChannel(0)
		
        exports.tokovoip_morp:removePlayerFromRadio(PhoneData.CallData.CallId)
    end
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        exports.tokovoip_morp:addPlayerToRadio(PhoneData.CallData.CallId, 'Phone')
    else
        PhoneData.CallData.InCall = false
        PhoneData.CallData.CallType = nil
        PhoneData.CallData.AnsweredCall = false
...
AddEventHandler('MI-phone:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = Lang("BANK_TITLE"),
                text = "€"..amount.." Sono stati ritirati dal conto bancario!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = Lang("BANK_TITLE"),
                content = "€"..amount.." Sono stati ritirati dal tuo conto bancario!", 
                icon = "fas fa-university",
                timeout = 3500, 
                color = "#ff002f",
            },
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('MI-Lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('MI-Lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('MI-Lapraces:server:StartRace', data.RaceData.RaceId)
end)
...
RegisterNUICallback('GetRaces', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('MI-Lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['MI-Lapraces']:IsInRace()
    print(InRace)
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:IsAuthorizedToCreateRaces', function(NameAvailable)
        local data = {
            IsAuthorized = true,
            IsBusy = exports['MI-Lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('MI-Lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('MI-Lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['MI-Lapraces']:IsInEditor())
    else
        cb(exports['MI-Lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    ESX.TriggerServerCallback('MI-Lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)
...
RegisterNUICallback('GetPlayerKeys', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-houses:server:getHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('GetCurrentMedic', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCurrentMedic', function(medic)
        cb(medic)
    end)
end)

RegisterNUICallback('GetCurrentpolice', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCurrentpolice', function(police)
        cb(police)
    end)
end)

RegisterNUICallback('GetCurrentcardealer', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCurrentcardealer', function(cardealer)
        cb(cardealer)
    end)
end)

RegisterNUICallback('GetCurrentmechanic', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCurrentmechanic', function(mechanic)
        cb(mechanic)
    end)
end)


RegisterNUICallback('GetCurrentAmmu', function(data, cb)
    ESX.TriggerServerCallback('MI-phone:server:GetCurrentAmmu', function(ammu)
        cb(ammu)
    end)
end)
