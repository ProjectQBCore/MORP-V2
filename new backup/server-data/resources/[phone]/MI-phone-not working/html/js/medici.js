...
SetupMeosHome = function() {
    $("#meos-app-name").html("Benvenuto " + MI.Phone.Data.PlayerData.charinfo.firstname + " " + MI.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.person-search-result', function(e){
    ...
    var weaponlicense = "No";
    if (ClickedPersonData.weaponlicense) {
        weaponlicense = "Si";
    }

    var OpenElement = '<div class="person-search-result-name">Name: '+ClickedPersonData.firstname+' '+ClickedPersonData.lastname+'</div> <div class="person-search-result-bsn">Phone: '+ClickedPersonData.phone+'</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Data di nascita: '+ClickedPersonData.birthdate+'</div> <div class="person-search-result-gender">Sesso: '+Gender+'</div> &nbsp; <div class="person-search-result-warned">Porto d\'armi: '+weaponlicense+'</div> <div class="person-search-result-driverslicense">Patente: '+HasLicense+'</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Name: '+PreviousPersonData.firstname+' '+PreviousPersonData.lastname+'</div> <div class="person-search-result-bsn">Telefono: '+PreviousPersonData.phone+'</div>';
        ...
        $.post('http://MI-phone/FetchVehicleResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function (i, vehicle) {
                    ...
                    var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">'+vehicle.label+'</div> <div class="vehicle-search-result-plate">Targa: '+vehicle.plate+'</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Proprietario: '+vehicle.owner+'</div> &nbsp; <div class="vehicle-search-result-apk">Assicurazione: '+APK+'</div> <div class="vehicle-search-result-warrant">Rubata: '+Flagged+'</div> </div>'
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NUOVO</span> <p class="meos-alert-type">Messaggio: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p> <hr> <div class="meos-location-button">POSIZIONE</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-'+randId+'"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NUOVO</span> <p class="meos-alert-type">Messaggio: '+data.alert.title+'</p> <p class="meos-alert-description">'+data.alert.description+'</p></div>';
    }
    $(".meos-recent-alerts").html('<div class="meos-recent-alert" id="recent-alert-'+randId+'"><span class="meos-recent-alert-title">Messaggio: '+data.alert.title+'</span><p class="meos-recent-alert-description">'+data.alert.description+'</p></div>');
