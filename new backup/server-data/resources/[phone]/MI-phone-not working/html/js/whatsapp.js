...
const monthNames = ["יָנוּאָר", "פברואר", "Marzo", "מרץ", "אפריל", "מאי", "יוני", "יולי", "אוגוסט", "ספטמבר", "אוקטובר", "נובמבר", "דצמבר"];
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "" && !Message.includes("'")) {
...
MI.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                if (message.type == "message") {
                    ...
                    MessageElement = '<div class="whatsapp-openedchat-message whatsapp-openedchat-message-'+Sender+' whatsapp-shared-location" data-x="'+message.data.x+'" data-y="'+message.data.y+'"><span style="font-size: 1.2vh;"><i class="fas fa-thumbtack" style="font-size: 1vh;"></i> Posizione</span><div class="whatsapp-openedchat-message-time">'+message.time+'</div></div><div class="clearfix"></div>'
                }
        ...
        var ChatDiv = '<div class="whatsapp-openedchat-messages-'+DateString+' unique-chat"><div class="whatsapp-openedchat-date">היום</div></div>';
