SetupMedic = function(data) {
    ...
    if (data.length > 0) {
        $.each(data, function(i, lawyer){
            ...
            $(".medic-list").append(element);
            $("#lawyerid-"+i).data('LawyerData', lawyer);
        });
    } else {
        var element = '<div class="lawyer-list"><div class="no-medic">No medics onduty.</div></div>'
        $(".medic-list").append(element);
    }
}
