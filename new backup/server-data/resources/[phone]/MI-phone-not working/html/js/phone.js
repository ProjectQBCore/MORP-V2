...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('http://MI-phone/CallContact', JSON.stringify({
        ...
        if (cData.number !== MI.Phone.Data.PlayerData.charinfo.phone_number) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== MI.Phone.Data.PlayerData.charinfo.phone_number) {
...
$(document).on('click', '#edit-contact', function(e){
    ...
    $(".phone-edit-contact-header").text(ContactData.name+"")
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!MI.Phone.Data.CallActive) {
            ...
            MI.Phone.Animations.TopSlideUp('.'+MI.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "יש לך שיחה נכנסת מ "+CallData.name
                if (AnonymousCall) {
                    Label = "יתקשר אליך על ידי מספר אנונימי"
                }
                $(".call-notifications-title").html("שיחה נכנסת");
