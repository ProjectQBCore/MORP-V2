    ...
    while true do
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~o~E~w~ - To shop for clothes')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~o~E~w~ - To adjust')
                            end
    ...
    while true do
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                if dist < 15 then
                        ...
                        if dist < 2 then
                            if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~o~E~w~ - Clothing')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    customCamLocation = Config.ClothingRooms[k].cameraLocation
                                    sex = "male"
                                    if ESX.GetPlayerData().sex == m then sex = "female" end
                                    ESX.TriggerServerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Presets", selected = true, outfits = Config.Outfits[ESX.PlayerData.job.name]},
...
AddEventHandler('qb-clothes:client:CreateFirstCharacter', function()
        ...
        if ESX.GetPlayerData().sex == m then 
            skin = "mp_f_freemode_01" 
		elseif ESX.GetPlayerData().sex == f then 
            skin = "mp_f_freemode_01"
        end
...
RegisterNetEvent('qb-clothing:client:loadPlayerClothing1')
AddEventHandler('qb-clothing:client:loadPlayerClothing1', function(data, ped)
    if ped == nil then ped = GetPlayerPed(-1) end
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        TriggerEvent('notification', ('Changed!'), 2)
    end
end)
