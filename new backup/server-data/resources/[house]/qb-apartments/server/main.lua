...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('notification', src,('You have a flat ('..label..')'), 2)
    TriggerClientEvent("apartments:client:SpawnInApartment", src, apartmentId, type)
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
    else
    TriggerClientEvent('notification', src,('You Do Not Have Enough Money In The Bank. Required Amount: $10000'), 2)
    end
end)
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    ...
    TriggerClientEvent('notification', src,('You changed your apartment!'), 2)
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
ESX.RegisterServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
ESX.RegisterServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
ESX.RegisterServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
ESX.RegisterServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
ESX.RegisterServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = ESX.GetPlayerFromId(src)
    if Player ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM apartments WHERE  identifier = @identifier",{['@identifier'] = Player.identifier}, function(result)
...
ESX.RegisterServerCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = ESX.GetPlayerFromId(src)
