	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				TriggerEvent('notification', ('You went out of range'), 2)
			end
		end
	end
end)
...
AddEventHandler("qb-houses:client:decorate", function()
	...
	if inside then 
		if hasKey then 
			...
			TriggerEvent('notification', ('You must have the keys to the house'), 2)
		end
	else
		TriggerEvent('notification', ('You are not in the house'), 2)
	end
end)
...
function CheckObjMovementInput()
    ...
    if IsControlPressed( 1, Keys["DELETE"]) or IsDisabledControlPressed(1, Keys["DELETE"]) then
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		TriggerEvent('notification', ('Speed set: '.. tostring(speeds[curSpeed])), 2)
	end
