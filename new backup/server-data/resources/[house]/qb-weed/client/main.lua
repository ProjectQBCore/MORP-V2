...
AddEventHandler('qb-weed:client:getHousePlants', function(house)    
   ESX.TriggerServerCallback('qb-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
    ...
    while true do
        ...
        if insideHouse then
            if plantSpawned then
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    local gender = "M"
                    if housePlants[currentHouse][k].gender == "woman" then gender = "W" end
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Kind: '..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Nutrition: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Can: ~b~'..plantData["plantStats"]["health"]..'%')
                            else
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"] + 0.2, 'The plant can be collected ~o~E~w~ press the key..')
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Kind: ~g~'..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Nutrition: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Can: ~b~'..plantData["plantStats"]["health"]..'%')
                                if IsControlJustPressed(0, QBWeed.Keys["E"]) then
									...
									local finished = exports["taskbar"]:taskBar(5000,"Removing Plant..")
									...
									TriggerEvent('notification', ('The plant have been removed..'), 2)
									end
                                end
                            end
                        elseif plantData["plantStats"]["health"] == 0 then
                            QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'The plant is dead, ~r~E~w~ - remove with key.')
                            if IsControlJustPressed(0, QBWeed.Keys["E"]) then
							   local finished = exports["taskbar"]:taskBar(5000,"Removing Plant..")
							   if finished == 100 then
							   TriggerEvent('notification', ('The plant have been removed..'), 2)
...
AddEventHandler('qb-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
		local finished = exports["taskbar"]:taskBar(5000,"Planting the seed..")
							   if finished == 100 then
							   TriggerServerEvent('qb-weed:server:placePlant', currentHouse, json.encode(plantData["plantCoords"]), type)
                TriggerEvent('notification', ('you have planted the seed..'), 2)
           end
        else
            TriggerEvent('notification', ('Not a good place..'), 2)
        end
    else
        TriggerEvent('notification', ('This place is not safe..'), 2)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    TriggerEvent('notification', ('The plant doesnt need nourishment..'), 2)
                else
				    local finished = exports["taskbar"]:taskBar(5000,"Feeding..")
							   if finished == 100 then
							   TriggerServerEvent('qb-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
							   TriggerEvent('notification', ('You have feeded the plant'), 2)
					end
                  
                end
            else
                TriggerEvent('notification', ('Not a plant..'), 2)
            end
        else
            TriggerEvent('notification', ('Not a plant..'), 2)
        end
